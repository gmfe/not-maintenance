...
const { cssInjection } = require('./compile-util/styleUtil');
const transformLess = require('./compile-util/transformLess');
const tsConfig = require('./compile-util/getTSCommonConfig')();
const getBabelCommonConfig = require('./compile-util/getBabelCommonConfig');
...
module.exports = function compileTs(modules, srcObject, dirName, isWatchMode, onFinish) {
  ...
  const sourceObject = mksureSrcObject || {
    less: [`src/${dirName}/**/*.less`],
    asset: [`src/${dirName}/**/*.@(png|svg)`],
  ...
  const changeDestinationForSingle = (files) => {
    if (isWatchMode) {
      const pathSplited = files[0].split('/');
      pathSplited.shift();
      pathSplited.pop();
      // NOTICE
      compDestination = `${destination}/src/${pathSplited.join('/')}`;
    }
  }

  if (sourceObject.less.length) {
    changeDestinationForSingle(sourceObject.less);
    const less = compileLess(sourceObject.less, compDestination);
    mergeResult.push(less);
  }

  if (sourceObject.asset.length) {
    changeDestinationForSingle(sourceObject.asset);
    const assets = compileAsset(sourceObject.asset, compDestination);
    mergeResult.push(assets);
  }
  if (sourceObject.thirdjs.length) {
    changeDestinationForSingle(sourceObject.thirdjs);
    const thirdjs = gulp.src(sourceObject.thirdjs).pipe(gulp.dest(destination + '/src/third-js'));
  ...
  if (sourceObject.source.length) {
    ...
    // allow jsx file in src/component/xxx/
    if (tsConfig.allowJs) {
    ...
    changeDestinationForSingle(source);
    if (isWatchMode && source.length === 1) {
    ...
    const tsResult = gulp
      .src(source)
      // .src(['src/component/**/*.tsx', 'src/component/**/*.ts', 'src/typings/**/*.d.ts'])
      .pipe(
        ts(tsConfgObject, {
          error(e) {
    ...
    tsResult.on('finish', () => {
      console.log(`[gm-tool-compile]: ts compile finish`);
      check();
      if (onFinish) {
        onFinish();
      }
    ...
    tsResult.on('end', () => {
      console.log(`[gm-tool-compile]: ts compile end`);
      check();
    });
  ...
  if (sourceObject.source.length === 0) {
    if (onFinish) {
      onFinish();
    }
  }
